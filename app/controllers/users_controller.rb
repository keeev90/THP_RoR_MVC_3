class UsersController < ApplicationController
  ...
  def new # pointer vers la view new (formulaire d'inscription)
    @city_options = City.all.map { |city| [city.name] }.flatten # transmettre données à la liste de choix déroulant ou @city_options = ["Paris", "Bordeaux", "Marseille"] # 
  end 
  ...
  def create
    ...
    # A la mano : @user.city = City.all.sample
